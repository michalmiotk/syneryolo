...
class Losses():
    ...
    def get_loc_error(predictions, targets):
        ...
        for y,x,pred in Losses.some_iterator(predictions):
            ...
            for i, tar in enumerate(targets.get((y,x),[])):
                ypred, xpred = tar[2]
                if i == 0:
    ...
    def get_wh_err_equation(wpred,wtar, hpred,htar):
        errw = Losses.lambda_coord*torch.pow(torch.sqrt(wpred)-torch.sqrt(torch.Tensor([wtar]).cuda()),2)
        errh = Losses.lambda_coord*torch.pow(torch.sqrt(hpred)-torch.sqrt(torch.Tensor([htar]).cuda()),2)
        return errw + errh
    @staticmethod
