...
some_predictions_for_loc_error = torch.zeros((1,7,7,30))
for y in range(7):
    for x in range(7):
...
some_predictions_class_prob_exist = torch.zeros((1,7,7,30))
for y in range(7):
    for x in range(7):
        if y == 3 and x == 3:
...
some_predictions_class_prob_notexist = torch.zeros((1,7,7,30))
for y in range(7):
    for x in range(7):
        some_predictions_class_prob_notexist[0,y,x,:] = torch.ones((30))

unittest.TestCase().assertAlmostEqual(490, Losses.get_conditional_class_prob_notexist(some_predictions_class_prob_notexist, some_targets_class_prob_notexist).cpu().detach().numpy()[0], 2)
#0.5(noobject_coef*20objects*7*7) == 490

target_for_wh = {(3,3):[[0, [torch.Tensor([0.36]).cuda(),torch.Tensor([0.64]).cuda()], [0, 0]]]} 
predictions_wh = torch.zeros((1,7,7,30))
for y in range(7):
    for x in range(7):
        if y == 3 and x == 3:
